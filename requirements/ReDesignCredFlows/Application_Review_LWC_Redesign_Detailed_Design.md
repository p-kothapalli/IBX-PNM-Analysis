...
> **🔄 COMBINED FLOW (business-ratified 2026-06-27):** Application Review is **no longer a standalone flow**. Business confirmed App Review and **Initial Cred PSV Review** are performed together for initial credentialing, so the two are **merged into one "Initial Credentialing Review" dashboard** — see `MASTER_Development_Plan_Credentialing_LWC_Redesign.md` (Flow 1) for the authoritative combined plan. **This document is retained as the per-tile reference for the App-Review-origin tiles** (Practitioner Info merge, Taxonomy, Education, License/SBRD, DEA, CDS, Work History, Malpractice, File Upload, Final Submit).
>
> **What changed for this doc's tiles in the combined flow:**
> - **Review Application** tile is **merged with PSV's Practitioner Info** into a single `prm_practitionerInfo` tile (identity/demographics + PSV-extended fields: Gender, Email, Degree, Telehealth).
> - **Verify Taxonomy** is the single shared `prm_taxonomyVerification` tile (App Review's separate tile = PSV's embedded taxonomy).
> - **File Upload** and **Final Submit** are the shared `prm_fileUpload` / `prm_finalSubmit` (merged with PSV's Final Confirmation; one `PRM_ReviewPSVCaseRecordsUpdate` IP call for the whole submission).
> - `Flow_Type__c = InitialCredReview` (replaces the former `ApplicationReview`); controller is the combined `CredentialingReviewController` (replaces `ApplicationReviewController`).
> - Education, License/SBRD, DEA, CDS, Work History, Malpractice carry over **unchanged** as combined-flow tiles 3-8.
